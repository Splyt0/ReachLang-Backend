...
export const main = Reach.App(() => {
  const Split00 = Participant("Creator", {
    ...
    createGroup: Fun(
      [
        Array(Address, 5),
        Array(
          Object({ UInt, Address, Address, UInt, expenseStatus, Map, Map }),
          100
        ),
      ],
      Null
  ...
  const [expenseId, payer, creator, totalExpense, status, balances, settled] =
    parallelReduce(
      expenseId,
      payer,
      creator,
    ...
    .api_(
      ...
      (expenseId, payer, creator, totalExpense, status, balances, settled) => {
        members[this] = isSome(members[this]) ? members[this] + totalExpense : totalExpense
